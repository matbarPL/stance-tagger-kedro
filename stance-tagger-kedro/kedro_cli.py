...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/stance_tagger_kedro")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=stance_tagger_kedro"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Stance Tagger Kedro"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "stance_tagger_kedro"
